...
FED_Rates$DFF <- as.numeric(FED_Rates$DFF)
head(FED_Rates)
nrow(FED_Rates)
#starting from 2007, 1st row 2007
FED_Rates
sum(is.na(FED_Rates$DFF))
FED_Rates <- FED_Rates[nrow(FED_Rates):1, ]
library(tidyr)
library(dplyr)
library(lubridate)
# Create a sequence of dates from Oct 1, 2007, to Sep 5, 2023
date_sequence <- seq(as.Date("2007-10-01"), as.Date("2023-09-25"), by = "day")

# Convert your 'Date' column to a date format
FED_Rates$Date <- as.Date(FED_Rates$Date)

# Create a complete dataset with all dates in the sequence
complete_dates <- tibble(Date = date_sequence)

# Merge the complete dates with your dataset
filled_data <- complete_dates %>%
  left_join(FED_Rates, by = "Date")

# Fill missing values in DFF with next or previous value
filled_data$DFF <- filled_data %>%
  group_by(Date) %>%
  fill(DFF, .direction = c("next", "previous"))
...
merged_data_fed <- merged_data_fed %>%
  rename(
    Nasdaq = Price.x,
    USDIndex = Price.y,
    US10Yearbond = Price,
...
library(GGally)
pair_data <- merged_data[, c("Nasdaq", "USDIndex", "US10Yearbond", "Inflation", "LIBOR")]
ggpairs(pair_data)
```
```{r}
merged_data$Nasdaq <- as.numeric(gsub(",", "", merged_data$Nasdaq))
...
```{r}
#///merged_data_fed$Date <- as.Date(merged_data_fed$Date)
#merged_data_fed <- na.omit(merged_data_fed)
#set.seed(123)
#train_indicesf <- sample(1:nrow(merged_data_fed), 0.8 * nrow(merged_data_fed))
#train_dataf <- merged_data_fed[train_indicesf, ]
#test_dataf <- merged_data_fed[-train_indicesf, ]
#train_dataf <- na.omit(train_dataf)
...
# Predict Nasdaq using the test set
predictions_rf1 <- predict(model_rf1, newdata = test_data)
...
#[1] "Random Forest RMSE: 191.129406950224"
# Feature Importance
...
```{r}
# Predict Nasdaq using the test set
predictions_A1 <- predict(model_A1, newdata = test_data_A)
...
```{r}
# Predict Nasdaq using the test set
predictions_A2 <- predict(model_A2, newdata = test_data_A)
...
predictions_apple <- predict(model_A_rf1, newdata = test_data)
...
# Predict Nasdaq using the test set
predictions_apple_rf1_time <- predict(model_apple_rf1_time, newdata = test_apple_time)
...
```
```{r}
predictions_ET1 <- predict(modelET, newdata = test_data)
...
print(modelET_RF)
predictions_RF_ET <- predict(modelET_RF, newdata = test_data)
