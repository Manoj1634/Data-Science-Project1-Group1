...
```{r}
ggplot(nasdaq_data, aes(x = Open, y = Price)) +
  geom_point() +
  geom_smooth(method = "lm", color = "green") +
  labs(title = "Nasdaq - Scatter Plot of Open vs. Price", x = "Opening Price", y = "Closing Price") +
  theme_minimal()
```
...
#Before creating the plots, you need to extract time-based features like month or year from your date column. Here's how you can do it in R:

test_nasdaq$Month <- format(as.Date(test_nasdaq$Date), "%m")
test_nasdaq$Year <- format(as.Date(test_nasdaq$Date), "%Y")

### 2. Month/Year-wise Aggregate Analysis

#Plotting the average price by month or year can show seasonal or yearly trends.

#### Average Price by Month
# For Nasdaq
ggplot(test_nasdaq, aes(x = Month, y = Price)) +
  stat_summary(fun = mean, geom = "bar", fill = "lightblue") +
  labs(title = "Average Price by Month - Nasdaq", y = "Average Price") +
  theme_minimal()
...
```{r}
# For Nasdaq
ggplot(test_nasdaq, aes(x = Year, y = Price)) +
  stat_summary(fun = mean, geom = "bar", fill = "lightblue") +
  labs(title = "Average Price by Year - Nasdaq", y = "Average Price") +
  theme_minimal()
```

```{r}
##Regression with Time-Based Predictors

#You can also include these time-based features in your regression model and then plot the coefficients to show their significance.

# Example Linear Regression with Time Features
library(coefplot)
model <- lm(Price ~ Month + Year, data = test_nasdaq)
summary(model)  # Check the significance of the coefficients
