...
```{r}
ggplot(nifty50_data, aes(x = Open, y = Price)) +
  geom_point() +
...
test_nifty$Month <- format(as.Date(test_nifty$Date), "%m")
test_nifty$Year <- format(as.Date(test_nifty$Date), "%Y")
...
ggplot(test_nifty, aes(x = Month, y = Price)) +
  stat_summary(fun = mean, geom = "bar", fill = "lightblue") +
  labs(title = "Average Price by Month - Nifty50", y = "Average Price") +
  theme_minimal()
```

```{r}
# For Nasdaq
ggplot(test_nifty, aes(x = Year, y = Price)) +
  stat_summary(fun = mean, geom = "bar", fill = "lightblue") +
  labs(title = "Average Price by Year - Nifty50", y = "Average Price") +
  theme_minimal()
```
...
library(coefplot)
model <- lm(Price ~ Month + Year, data = test_nifty)
